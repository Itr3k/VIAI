...
import GlobalIntegrationsPage from './components/admin/GlobalIntegrationsPage';
...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<AgencySettings>({ name: 'Acme Agency', nangoConnected: false, korraEnabled: true });
  ...
  if (userRole === 'super_admin' && !isImpersonating) {
    return (
      <div className="min-h-screen flex bg-slate-50">
        ...
        {systemNotification && systemNotification.active && (
          <div className={`fixed top-0 left-0 right-0 z-50 px-4 py-2 flex items-center justify-between shadow-md ${systemNotification.type === 'error' ? 'bg-red-600 text-white' :
            systemNotification.type === 'warning' ? 'bg-amber-500 text-white' :
        ...
        <aside className={`hidden md:flex w-64 bg-slate-900 text-slate-300 flex-col fixed h-full z-10 transition-all duration-300 ${systemNotification ? 'mt-10' : 'inset-y-0'}`}>
          <div className="p-6">
            {isImpersonating && impersonatedAgency?.logoUrl ? (
              <img src={impersonatedAgency.logoUrl} alt={impersonatedAgency.name} className="h-8 object-contain mb-2" />
            ) : (
              <h1 className="text-xl font-bold text-white tracking-tight">
                VIAI <span className="text-brand-400">.</span>
              </h1>
            )}
            <p className="text-xs text-brand-400 mt-2 font-mono uppercase">
              {isImpersonating ? `Viewing: ${impersonatedAgency?.name}` : 'Super Admin'}
            </p>
          </div>
          <nav className="flex-1 px-4 mt-4 space-y-2">
            <button
              onClick={() => setView('dashboard')}
              className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'dashboard' ? 'bg-red-900/50 text-white shadow-inner' : 'hover:bg-slate-800'}`}
            >
            ...
            <button
              ...
              className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'agencies' ? 'bg-red-900/50 text-white shadow-inner' : 'hover:bg-slate-800'}`}
            >
              <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
              Agencies
            </button>
            <button
              ...
              className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'clients' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/50' : 'hover:bg-slate-800'}`}
            >
            ...
            <button
              ...
              className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'settings' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/50' : 'hover:bg-slate-800'}`}
            >
            ...
            <button
              ...
              className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'support' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/50' : 'hover:bg-slate-800'}`}
            >
        ...
        <main className={`ml-0 md:ml-64 flex-1 p-8 overflow-y-auto h-screen ${systemNotification ? 'mt-10' : ''}`}>
          {view === 'dashboard' && (
            <SuperAdminPage
              agencies={MOCK_AGENCIES}
              onImpersonate={(agency) => startImpersonation(agency, {
                ...settings, // Assuming settings is defined elsewhere and we're spreading it
                korraEnabled: true, // Adding korraEnabled to the settings object passed to impersonation
              })}
              onUpdateNotification={setSystemNotification}
          ...
          {view === 'users' && (
            <UsersPage currentUserRole="super_admin" agencyId="super_admin" />
          )}
          {view === 'clients' && (
          ...
          {view === 'calls' && !selectedCall && (
            <CallsPage
              calls={calls} // In a real app, filter for Super Admin's clients
              onSelectCall={setSelectedCall}
            />
          )}
          {view === 'integrations' && <IntegrationsPage />}
          {view === 'settings' && <SettingsPage />}
          {view === 'support' && <SupportPage currentUserRole="super_admin" agencyId="super_admin" />}
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 flex-col md:flex-row">
      {/* Global Notification Banner */}
      {systemNotification && (
        <div className={`fixed top-0 left-0 right-0 z-50 p-3 text-sm font-medium text-white shadow-md flex justify-between items-center animate-in slide-in-from-top duration-300 ${systemNotification.type === 'error' ? 'bg-red-600' :
          systemNotification.type === 'warning' ? 'bg-amber-500' :
            'bg-blue-600'
          }`}>
          <div className="flex items-center gap-2 text-sm font-medium mx-auto">
            <AlertCircle className="w-4 h-4" />
          ...
          <button
            id="dismiss-banner-btn-agency"
            onClick={() => setSystemNotification(null)} // Local dismiss for user
            className="text-white/80 hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
      ...
      <aside className={`hidden md:flex w-64 bg-slate-900 text-slate-300 flex-col fixed inset-y-0 z-10 transition-all duration-300 ${isImpersonating ? 'top-9' : 'top-0'}`}>
        <div className="p-6">
          {settings.logoUrl ? (
            <img src={settings.logoUrl} alt={settings.name} className="h-10 object-contain mb-2" />
          ) : (
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
          ...
          <button
            onClick={() => { setView('clients'); setSelectedCall(null); }}
            className={`flex items-center w-full px-4 py-3 rounded-lg transition-all ${view === 'clients' ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/50' : 'hover:bg-slate-800'}`}
          >
            <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
            Clients
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          {!isImpersonating && (
            <div className="flex flex-col gap-4">
              ...
              {userRole !== 'super_admin' && (
                <div>
                  <label className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-2 block">Switch Agency</label>
                  <select
                    className="w-full bg-slate-800 text-slate-300 text-xs rounded border border-slate-700 p-2 focus:outline-none focus:border-brand-500"
                    value={settings.name}
      ...
      <main className={`ml-0 md:ml-64 flex-1 p-8 overflow-y-auto h-screen ${isImpersonating ? 'mt-9' : ''}`}>
        ...
        {view === 'clients' && !selectedCall && <ClientsPage />}
        ...
        {view === 'settings' && !selectedCall && <SettingsPage settings={settings} onUpdate={setSettings} />}
        {view === 'support' && !selectedCall && <SupportPage currentUserRole="admin" agencyId="agency-1" />}
